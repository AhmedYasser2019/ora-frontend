...
export function ProductCard({ p, price }: { p: Product; price?: number | undefined }) {
  ...
  return (
    <article className="flex flex-col overflow-hidden rounded-2xl border border-border bg-card transition-shadow hover:shadow-soft">
      ...
      >
        {!p.available && (
          <span className="absolute right-2 top-2 z-10 rounded-full bg-destructive px-2.5 py-1 text-[10px] font-semibold text-destructive-foreground">
            غير متوفر
          </span>
        )}
        <img
      ...
      <div className="flex flex-1 flex-col p-4">
        ...
        <p className="mt-1 text-[11px] text-gold-deep">{p.provider}</p>
        <div className="mt-3 flex flex-1 items-end justify-between gap-2">
          ...
          >
            ...
            {added ? "تمت الإضافة" : p.available ? "أضف للسلة" : "غير متوفر"}
          </button>
        </div>
      </div>
    </article>
