...
## ✅ 6. خطوة OTP في التسجيل — 2026-09-14
- اتعمل: الخطوة اتشالت من `src/routes/auth.tsx` (التسجيل بقى 3 خطوات: البيانات ← الهوية ← الخبرة) ونصوصها اتشالت من `i18n.en.ts`. مينفعش تتربط دلوقتي: `sendOtp` بيطلع كود للمستخدمين الموجودين بس (استعادة كلمة المرور)، ومفيش بوابة SMS.
- لما البوابة تيجي: محتاج endpoint تحقق للتسجيل في الباك إند (رقم مش متسجل) + `register` يرفض من غيره، وبعدين نرجّع الخطوة.
...
## ✅ 7. نسيت كلمة المرور — 2026-09-14
- اتعمل: لينك «نسيت كلمة المرور؟» تحت فورم الدخول في `src/routes/auth.tsx` بيفتح `ForgotPassword` (الموبايل ← الرمز ← كلمة مرور جديدة ≥ 8) على الـ 3 endpoints. الرسالة مش بتكشف لو الرقم مسجَّل، ولو الـ reset token انتهى بيرجع لأول خطوة، وفي وضع debug الرمز بيتملى لوحده. كمان حد التسجيل الأدنى بقى 8 بدل 6 (الباك إند `min:8`). الترجمة في `i18n.en.ts`.
- **باقي:** إجابة FAQ «نسيت كلمة المرور» في الداشبورد بتقول «أدخل بريدك وهيوصلك رابط» — غلط، تتعدل من الداشبورد لـ رقم الموبايل ورمز التحقق.
